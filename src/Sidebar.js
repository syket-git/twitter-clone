  ...
  return (
    <div className="sidebar">
      ...
      <div className="sidebar__profileInfo">
        ...
        />
        <div>
          <h4 className="sidebar__profileInfo__displayName">{auth.user?.displayName}</h4>
  <p className="sidebar__profileInfo__username">{"@" + auth.user?.displayName.replace(/\s/g, '').toLowerCase()}</p>
        </div>
      </div>
    </div>
  );
